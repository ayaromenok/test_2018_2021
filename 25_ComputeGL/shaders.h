...
static const char *csComputeSource =
        ...
        "  int top   = clamp(y - radiusY, 0, imgSize.y - 1);\n"
        "  int bottom  = clamp(y + radiusY, 0, imgSize.y - 1);\n"
        "  for (int iX = left; iX <= right; iX++) {\n"
        "     for (int iY = top; iY <= bottom; iY++) {\n"
        "        vec4 imgValue =  imageLoad(inputImage, ivec2(iX,iY));\n"
        "        sumPixels += (imgValue * weightX*weightY);\n"
        "     }//for\n"
        "  }//for\n"

...
int main(int, char**)
{
    ...
    capture.set(CAP_PROP_FRAME_WIDTH, 672);
    capture.set(CAP_PROP_FRAME_HEIGHT, 380);
    VideoCapture capture2("/dev/video4", CAP_V4L); // second camera on rk3399 is /dev/video4
    capture2.set(CAP_PROP_FRAME_WIDTH, 672);
    capture2.set(CAP_PROP_FRAME_HEIGHT, 3980);
    if (!capture.isOpened())
    {
